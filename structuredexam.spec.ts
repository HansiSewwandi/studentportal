...
test('Structured Exam - Upload and Submit Answer Sheet', async ({ page }) => {
  // 1. Authenticate user
  await login(page);

  // 2. Navigate & Start Exam
 ...
 const ongoingText = page.getByText(/Ongoing/i).first();
  ...
  // 3. Upload Answer Document & Submit

const fileInput = page.locator('#studentFileInput');
  if (await fileInput.count() > 0) {
    await fileInput.setInputFiles('AS new.pdf').catch(() => {});
  }
